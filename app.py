###############################################################################
# FitBites – full app (2025-05-13)  · updated: calories in classic plan, fixed registration, reshuffle, profile save
###############################################################################
...
def save_user(u,p):
    ...
    df = pd.concat([df, pd.DataFrame([[u,p]], columns=["username","password"])])
    ...
    for d in range(1, 8):
        ...
        for meal, f in split.items():
            ...
            for s in prefs.get(meal, []):
                opts.extend(similar(s, exc=dislikes))
            if not opts:
                opts = list(set(df.Food.unique()) - set(dislikes))
            if not opts:
                pick = "No food available"
                meal_kcal = 0
            else:
...
def gpt_plan(prefs, dislikes, kcal):
    ...
    rules = (
        ...
        "• There is no such thing as snails soup or crabs soup. So don't recommend non-existent Ghanaian foods.\n"
...
with tab_plan:
    ...
    if col1.button("✨ Generate / Update"): gen_plan(False)
    if col2.button("➕ Next week"): gen_plan(True)
    ...
    if st.session_state.meal_plan is not None:
        ...
        st.download_button("⬇️ Download Plan", st.session_state.meal_plan.to_csv(index=False).encode(),
                           file_name=f"mealplan_week{st.session_state.current_week}.csv")
        if st.button("🔄 Reshuffle"): st.session_state.show_reshuffle = True

    def ensure_kcal():  # fallback if calories unset
...
""")
        if st.button("💾 Save Profile Again"):
